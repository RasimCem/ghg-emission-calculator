class Player {
    ...
    constructor(list, element) {
        ...
        $(document.body).find($(this.target).find(".playpause-track")).click(function () {
            if ($(this).attr("class").split(/\s+/).includes("play-track")) {
                ...
                $(this).html('<images class="fa-solid fa-pause"></images>')
            }
            else {
                _this.pause()
                $(this).html('<images class="fa-solid fa-play"></images>')
    ...
    setVolume(volume) {
        ...
        switch (parseFloat(volume)) {
            case 0:
                $(this.target).find(".volume__state").removeClass("setVolumeZero").html('<images class="fa-solid fa-volume-xmark"></images>')
                break
            case 0.2:
                $(this.target).find(".volume__state").addClass("setVolumeZero").html('<images class="fa-solid fa-volume-off"></images>')
                break
            case 0.4:
                $(this.target).find(".volume__state").addClass("setVolumeZero").html('<images class="fa-solid fa-volume-low"></images>')
                break;
            case 0.6: case 0.8: case 1:
                $(this.target).find(".volume__state").addClass("setVolumeZero").html('<images class="fa-solid fa-volume-high"></images>')
                break;
        }
    ...
    resetValue() {
        ...
        $(this.target).find(".playpause-track").addClass("play-track").html('<images class="fa-solid fa-play"></images>')
